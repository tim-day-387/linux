...
vdev_copy_path_search(vdev_t *stvd, vdev_t *dvd)
{
	...
	/*
	 * Allow transitioning from file vdev to disk vdev when the source
	 * has a disk vdev. This handles the case where a pool was created
	 * with a file-backed vdev and then the image was written to a block
	 * device (e.g., for root filesystem booting).
	 */
	if (vd->vdev_ops != dvd->vdev_ops) {
		if (vd->vdev_ops == &vdev_disk_ops &&
		    dvd->vdev_ops == &vdev_file_ops) {
			zfs_dbgmsg("vdev_copy_path_search: converting vdev "
			    "%llu from file to disk",
			    (u_longlong_t)dvd->vdev_guid);
			dvd->vdev_ops = &vdev_disk_ops;
		} else {
			return;
		}
	}
